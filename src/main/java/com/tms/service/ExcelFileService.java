...
public class ExcelFileService implements FileService {
	...
	public void generateAndSaveTimesheetReportExcelFile(String targetLocation, List<Timesheet> timesheets)
		...
		String headers[] = { "username", "project", "fromTime" , "toTime" , "loginDate", "loggedHr" };
		...
		for (Timesheet timesheet : timesheets) {
			...
			if(timesheet.getFromTime()==null) {
				row.createCell(3).setCellValue("");
			}else {
				row.createCell(3).setCellValue(timesheet.getFromTime().toString());	
			}
			if(timesheet.getToTime()==null) {
				row.createCell(4).setCellValue("");
			}else {
				row.createCell(4).setCellValue(timesheet.getToTime().toString());
			}
			row.createCell(5).setCellValue(timesheet.getLoggedHr());
		}
